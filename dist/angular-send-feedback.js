 ...
 * @version v1.0.2 - 2015-12-04 * @link https://github.com/jacobscarter/angular-feedback
    ...
    return {
        ...
        link: function ($scope) {
            ...
            var settings = _.extend({
                ...
                lineJoin: 'bevel',
                lineWidth: 3,
                html2canvasURL: 'assets/vendor/js/html2canvas/html2canvas.js',
                ...
                tpl: {
                    ...
                    overview: '<div id="feedback-overview"><div class="feedback-logo">Feedback</div><div id="feedback-overview-description"><div id="feedback-overview-description-text"><h3>Subject</h3><h3>Description</h3><h3 class="feedback-additional">Additional info</h3><div id="feedback-additional-none"><span>None</span></div><div id="feedback-browser-info"><span>Browser Info</span></div><div id="feedback-page-info"><span>Page Info</span></div><div id="feedback-page-structure"><span>Page Structure</span></div></div></div><div id="feedback-overview-screenshot"><h3>Screenshot</h3></div><div class="feedback-buttons"><button id="feedback-submit" class="feedback-submit-btn feedback-btn-blue">Submit</button><button id="feedback-overview-back" class="feedback-back-btn feedback-btn-gray">Back</button></div><div id="feedback-overview-error">Please enter a description.</div><div class="feedback-wizard-close"></div></div>',
            ...
            if (supportedBrowser) {
                if (isFeedbackButtonNative) {
                    $('body').append('<button class="feedback-btn" ng-click="vm.init()" style="position: fixed; bottom: 0px; right:0px; z-index: 999; transform: rotate(-45deg) translateY(40px) translateX(25px);font-size: 35px; width: 140px;height: 65px;"><i class="fa fa-fw fa-bug"></i></button>');
                }
                $(document).on('click', settings.feedbackButton, function () {
                        ...
                        html2canvas($('body'), {
                            onrendered: function (canvas) {
                                ...
                                if (settings.showDescriptionModal) {
                                    ...
                                    $('<textarea id="feedback-overview-subject">' + ' </textarea>').insertAfter('#feedback-overview-description-text h3:eq(0)');
                    ...
                    $(document).on('keyup', '#feedback-note-tmp,#feedback-note-subject,#feedback-overview-note,#feedback-overview-subject', function (e) {
                        var tx, sub;
                        if (e.target.id === 'feedback-note-tmp'){
                            tx = $('#feedback-note-tmp').val();
                            sub = $('#feedback-overview-subject').val();
                        } else {
                            tx = $('#feedback-overview-note').val();
                            sub = $('#feedback-overview-subject').val();
                            $('#feedback-note-tmp').val(tx);
                            $('#feedback-subject-tmp').val(sub);
                        }

                        $('#feedback-note').val(tx);
                        $('#feedback-subject').val(sub);
                    });
                    ...
                    $(document).on('click', '#feedback-submit', function () {
                        ...
                        if ($('#feedback-note').val().length > 0) {
                            ...
                            function submitScreenshot(request) {
                                var b64Data = request.attached_file.slice(request.attached_file.indexOf(',')+1);
                                ...
                                var blob = new Blob([byteArray], {type: 'image/png,base64'});
                                var file = new File([blob], 'screenshot.png');

                                var fd = new FormData();
                                fd.append('attached_file', file);
                                fd.append('object_id', request.object_id);
                                fd.append('project', request.project);
                            ...
                            function submitIssue(request){
                              request.description = request.description + ' ||| Informacje zalogowanym użytkowniku - id: ' + $rootScope.user.id + ', email: ' + $rootScope.user.email;
                              request.description = request.description + ' ||| Adres Url: ' + $location.absUrl();
                                $http({
                                    method: 'POST',
                                    url: taigaUrl + '/issues',
                            ...
                            submitIssue({
                                ...
                                subject: $('#feedback-overview-subject').val(),
